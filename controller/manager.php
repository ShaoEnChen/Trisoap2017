...
if (isset($_GET['in']) || isset($_POST['in'])) {
	...
	if ($in == 'create') {
		if (isset($_COOKIE['account']) && isset($_COOKIE['identity']) && $_COOKIE['identity'] == 'A') {
			include_once("view/function/managerCreate.html");
		}
	}
	elseif ($in == 'delete') {
		if (isset($_COOKIE['account']) && isset($_COOKIE['identity']) && $_COOKIE['identity'] == 'A') {
			include_once("view/function/managerDelete.html");
		}
	}
}

elseif (isset($_COOKIE['account']) && isset($_COOKIE['identity']) && $_COOKIE['identity'] == 'A') {
	$myfile = fopen("view/function/manager.html", "r");
	$content = fread($myfile, filesize("view/function/manager.html"));
	fclose($myfile);
