...
if (isset($_GET['order']) || isset($_POST['order'])) {
	...
	if (isset($_COOKIE['account']) && isset($_COOKIE['token'])) {
		$myfile = fopen("view/pay.html", "r");
		$content = fread($myfile, filesize("view/pay.html"));
		fclose($myfile);
		...
		if (empty($operate)) {
			$myfile = fopen("view/pay_empty.html", "r");
			$innerContent = fread($myfile, filesize("view/pay_empty.html"));
			fclose($myfile);
			$content = str_replace('[payContent]', $innerContent, $content);
		}
		else {
			$myfile = fopen("view/pay_content.html", "r");
			$innerContent = fread($myfile, filesize("view/pay_content.html"));
			fclose($myfile);
			$content = str_replace('[payContent]', $innerContent, $content);
